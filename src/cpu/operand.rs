...
use crate::{cpu::{Cpu, instructions::{step, go}}, peripherals::Peripherals};

pub trait IO8<T: Copy> {
    fn read8(&mut self, bus: &Peripherals, src: T) -> Option<u8>;
    fn write8(&mut self, bus: &mut Peripherals, dst: T, value: u8) -> Option<()>;
}

pub trait IO16<T: Copy> {
    fn read16(&mut self, bus: &Peripherals, src: T) -> Option<u16>;
    fn write16(&mut self, bus: &mut Peripherals, dst: T, value: u16) -> Option<()>;
}
...
impl IO8<Reg8> for Cpu {
    ...
    fn write8(&mut self, _: &mut Peripherals, dst: Reg8, value: u8) -> Option<()> {
       Some(
            match dst {
...
impl IO16<Reg16> for Cpu {
    ...
    fn write16(&mut self, _: &mut Peripherals, dst: Reg16, value: u16) -> Option<()> {
       Some(
            match dst {
                Reg16::AF => self.regs.write_af(value),
                Reg16::BC => self.regs.write_bc(value),
                Reg16::DE => self.regs.write_de(value),
                Reg16::HL => self.regs.write_hl(value),
...
impl IO8<Indirect> for Cpu {
    ...
    fn write8(&mut self, bus: &mut Peripherals, dst: Indirect, value: u8) -> Option<()> {
       step!(None, {
        0: {
            match dst {
                Indirect::BC => bus.write(self.regs.bc(), value),
                Indirect::DE => bus.write(self.regs.de(), value),
                Indirect::HL => bus.write(self.regs.hl(), value),
                Indirect::CFF => bus.write(0xFF00 | (self.regs.c as u16), value),
                Indirect::HLD => {
                    let addr = self.regs.hl();
                    self.regs.write_hl(addr.wrapping_sub(1));
...
impl IO8<Direct8> for Cpu {
    ...
    fn write8(&mut self, bus: &mut Peripherals, dst: Direct8, value: u8) -> Option<()> {
        step!(None, {
            0: if let Some(lo) = self.read8(bus, Imm8) {
                VAL8.store(lo, Relaxed);
                go!(1);
            ...
            2: {
                bus.write(VAL16.load(Relaxed), value);
                go!(3);
                return None;
            },
            3: return Some(go!(0)),
...
impl IO16<Direct16> for Cpu {
    ...
    fn write16(&mut self, bus: &mut Peripherals, dst: Direct16, value: u16) -> Option<()> {
        step!(None, {
            0: if let Some(lo) = self.read8(bus, Imm8) {
                VAL8.store(lo, Relaxed);
                go!(1);
            ...
            2: {
                bus.write(VAL16.load(Relaxed), value as u8);
                go!(3);
                return None;
            },
            3: {
                bus.write(VAL16.load(Relaxed).wrapping_add(1), (value >> 8) as u8);
                go!(4);
                return None;
            },
            4: return Some(go!(0)),
